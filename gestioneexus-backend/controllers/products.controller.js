...
const getUniqueBrands = async (req, res) => {
    try {
        ...
        // Mapeamos el resultado para enviar un array de strings simple
...
const createProduct = async (req, res) => {
    ...
    try {
        const [existing] = await pool.query(
            'SELECT * FROM products WHERE reference = ? AND sizes = ?',
            [reference, sizes]
        );

        if (existing.length > 0) {
            return res.status(400).json({ msg: `Ya existe un producto con la referencia '${reference}' y la talla '${sizes}'.` });
        }
...
module.exports = {
    ...
    deleteProduct,
    bulkImportProducts,
    getUniqueBrands // <-- Exportamos la nueva función
};