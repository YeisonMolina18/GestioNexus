...
// getLayawayPlans y getLayawayPlanById no cambian
...
const getLayawayPlanById = async (req, res) => {
    ...
    try {
        ...
        const [products] = await pool.query(
            `SELECT d.quantity, p.price as price_at_sale, p.name as product_name, p.id as product_id 
...
const createLayawayPlan = async (req, res) => {
    ...
    try {
        ...
        // --- CORRECCIÓN AQUÍ: Registramos el abono inicial en el libro contable ---
        if (down_payment > 0) {
            await connection.query(
...
const updateLayawayPlan = async (req, res) => {
    ...
    try {
        ...
        if (new_payment_amount && !isNaN(new_payment_amount)) {
            ...
            // --- CORRECCIÓN AQUÍ: Registramos el nuevo abono en el libro contable ---
            await connection.query(
                'INSERT INTO financial_ledger (entry_date, concept, income) VALUES (CURDATE(), ?, ?)',
